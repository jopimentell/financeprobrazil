...
import { Crown, ArrowRight, Sparkles, AlertTriangle } from 'lucide-react';
import { Link } from 'react-router-dom';

export default function SubscriptionPage() {
  const { currentPlan, currentLimits, currentSubscription, settings } = usePlan();
  ...
  return (
    <div className="space-y-6 animate-fade-in max-w-2xl mx-auto">
        ...
        <div className="flex items-center justify-between">
          ...
          {currentSubscription && (
            <span className={`px-2.5 py-1 rounded-full text-xs font-medium ${
              currentSubscription.status === 'active' ? 'bg-primary/10 text-primary' :
              currentSubscription.status === 'trial' ? 'bg-accent text-accent-foreground' :
              'bg-destructive/10 text-destructive'
            }`}>
              {currentSubscription.status === 'active' ? 'Ativo' :
               currentSubscription.status === 'trial' ? 'Trial' : 'Cancelado'}
            </span>
          )}
        </div>
        ...
        {currentSubscription && (
          <div className="grid grid-cols-2 gap-4 text-sm">
            <div>
              ...
              <p className="font-medium text-foreground">{new Date(currentSubscription.startDate).toLocaleDateString('pt-BR')}</p>
            </div>
            <div>
              <p className="text-muted-foreground">Próxima cobrança</p>
              <p className="font-medium text-foreground">{new Date(currentSubscription.endDate).toLocaleDateString('pt-BR')}</p>
            </div>
          </div>
        )}
      </div>
            ...
            <div key={item.label} className="space-y-1.5">
              <div className="flex justify-between text-sm">
                ...
                <span className={`font-medium ${isNearLimit ? 'text-destructive' : 'text-muted-foreground'}`}>
                  ...
                  {isNearLimit && !settings.monetizationEnabled ? '' : isNearLimit ? ' ⚠️' : ''}
                </span>
              </div>
      ...
      {currentPlan?.isFree && settings.monetizationEnabled && (
        <Link
          to="/plans"
          className="finance-card flex items-center justify-between hover:border-primary/30 transition-colors group"
        >
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">Desbloqueie mais contas, transações e relatórios</p>
            </div>
          </div>
