    ...
    <div className="space-y-4 animate-fade-in">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl flex items-center justify-center" style={{ backgroundColor: 'hsl(var(--finance-income) / 0.1)' }}>
            <ArrowDownCircle className="h-5 w-5 finance-income" />
          </div>
          <div>
            <h1 className="text-xl font-bold">Receitas</h1>
            <p className="text-sm text-muted-foreground">Total: <span className="finance-income font-semibold">{fmt(total)}</span></p>
          </div>
        </div>
            ...
            {monthNames.map((name, i) => {
              ...
              return (
                <div key={i} className="flex items-center justify-between py-3 border-b border-border/50 last:border-0">
                  ...
                  <span className="text-sm font-semibold finance-income">{fmt(inc)}</span>
                </div>
              );
            })}
          </div>
      ...
      <div className="finance-card !p-3 md:!p-5">
        ...
        <div className="mt-3 text-xs text-muted-foreground text-center">{incomes.length} receita(s)</div>
      </div>
